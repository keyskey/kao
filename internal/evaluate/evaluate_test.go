...
func TestEvaluateInfraDeploymentCM007Pass(t *testing.T) {
	...
	results := EvaluateAll(cfg, nil, nil, nil, []evidence.InfraDeployment{id}, join, id.AppliedAt)
...
func TestEvaluateInfraDeploymentCM007JoinFail(t *testing.T) {
	...
	results := EvaluateAll(cfg, nil, nil, nil, []evidence.InfraDeployment{id}, map[JoinKey]evidence.CodeChange{}, id.AppliedAt)
	cm007 := findResult(results, "CM-007")
...
func applyTestDefaults(cfg *config.Config) {
	...
	cfg.Controls.AppDeployment.Traceability.Required = &trueVal
